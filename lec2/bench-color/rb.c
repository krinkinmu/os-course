...
static bool rb_red(const struct rb_node *node)
{ return node && rb_color(node) == RED; }

static bool rb_black(const struct rb_node *node)
{ return !rb_red(node); }
...
static struct rb_node *rb_rightmost(struct rb_node *node)
{
	while (node->right)
		node = node->right;
	return node;
...
static struct rb_node *rb_leftmost(struct rb_node *node)
{
	while (node->left)
		node = node->left;
	return node;
